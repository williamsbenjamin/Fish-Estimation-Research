...
#non cls species, summed up for claim #Fix this.
...
tidyer_mrip <- tidy_mrip %>%
  ...
  select(1:22,(map(cls_species,    #select only the species seen in mrip alone
                   starts_with,
                   vars = colnames(.)) %>%
                 unlist())) %>% 
...
tidy_cls_16 <- tidy_cls %>% 
     filter(year(returndate)==2016)
write_csv(tidy_cls_16,"data/cls_tidy2_16.csv")
...
write_csv(tidy_cls_17,"data/cls_tidy2_17.csv")
...
fully_everything <- tidy_cls %>%
  drop_na(CLS_ID) %>%
  inner_join(tidyer_mrip,by = c("CLS_ID")) 
