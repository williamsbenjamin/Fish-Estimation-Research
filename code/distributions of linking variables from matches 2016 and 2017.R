...
#I keep the one with a smaller distance between reporting site 
#and dock (km_diff)

#these are the "perfect matches"
perfect_match <- tidy_all_matches_docks %>%
...
#now hold all but one variable constant to get distribution of 
#linking variables
#Start by holding anglers,state,date constant
...
#get frequencies of differences in total catch 215
tidy_all_matches_docks %>%
  filter(nbAnglers == PARTY,
         state == STATE,
         date == return_date_ymd) %>%
