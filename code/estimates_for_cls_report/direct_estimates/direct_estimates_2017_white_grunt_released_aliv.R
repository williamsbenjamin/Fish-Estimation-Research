...
mrip_all_17$`WHITE GRUNT_released`[is.na(mrip_all_17$`WHITE GRUNT_released`)] <- 0
mrip_all_17 <- mrip_all_17 %>% 
  mutate(delta_wg = `WHITE GRUNT_release` - `WHITE GRUNT_released`)
...
ty2_17_wg_total <- predict(ty2.r17_wg,total = nrow(cls_17))[[1]] + 
  sum(cls_17$`WHITE GRUNT_released`,na.rm=T)
...
mrip_all_17_1 <- read_csv("data/mrip_dm_17_all_sites.csv",
                          col_types = cols(psu_id = col_character()))
mrip_all_17_1$`WHITE GRUNT_released`[is.na(mrip_all_17_1$`WHITE GRUNT_released`)] <- 0
mrip_all_17_1 <- mrip_all_17_1 %>% 
  mutate(delta_wg= `WHITE GRUNT_release` - `WHITE GRUNT_released`) 
mrip_all_17_1_fl <- mrip_all_17_1 %>% 
  mutate(spanish_mackerel_release_fl = if_else(ST == 12,
                                             `WHITE GRUNT_release`,as.numeric(0)),
         spanish_mackerel_released_fl = if_else(ST == 12,
                                            `WHITE GRUNT_released`,as.numeric(0))) %>% 
  mutate(delta_wg_fl = spanish_mackerel_release_fl - spanish_mackerel_released_fl) %>% 
...
#ty2_17_cut_13_wg_se
ty2_17_cut_13_wg_fl_total <- predict(ty2.r17_wg_cut_13_fl,total = nrow(cls_17_fl))[[1]] + sum(cls_17_fl$`WHITE GRUNT_released`,na.rm=T)
...
mrip_all_17_2 <- read_csv("data/mrip_dm_17_all_sites.csv",
                          col_types = cols(psu_id = col_character()))
mrip_all_17_2$`WHITE GRUNT_released`[is.na(mrip_all_17_2$`WHITE GRUNT_released`)] <- 0
...
mrip_all_17_2_al <- mrip_all_17_2 %>% 
  mutate(spanish_mackerel_release_al = if_else(ST == 1,
         ...
         spanish_mackerel_released_al = if_else(ST == 1,
                                            `WHITE GRUNT_released`,as.numeric(0))) %>% 
  mutate(delta_wg_al = spanish_mackerel_release_al - spanish_mackerel_released_al) %>% 
...
#ty2_17_cut_13_wg_se
ty2_17_cut_13_wg_al_total <- predict(ty2.r17_wg_cut_13_al,total = nrow(cls_17_al))[[1]] + sum(cls_17_al$`WHITE GRUNT_released`,na.rm=T)
