...
mrip_all_17$`SPANISH MACKEREL_released`[is.na(mrip_all_17$`SPANISH MACKEREL_released`)] <- 0
mrip_all_17 <- mrip_all_17 %>% 
  mutate(delta_sm = `SPANISH MACKEREL_release` - `SPANISH MACKEREL_released`) %>%
  mutate(non_reporter_mrip_claim_sm = if_else(reported == 0,
                                           `SPANISH MACKEREL_release`,as.numeric(0)))
...
ty2_17_sm_total <- predict(ty2.r17_sm,total = nrow(cls_17))[[1]] + sum(cls_17$`SPANISH MACKEREL_released`,na.rm=T)
...
mrip_all_17_1 <- read_csv("data/mrip_dm_17_all_sites.csv",
                          col_types = cols(psu_id = col_character()))
mrip_all_17_1$`SPANISH MACKEREL_released`[is.na(mrip_all_17_1$`SPANISH MACKEREL_released`)] <- 0
...
mrip_all_17_1_fl <- mrip_all_17_1 %>% 
  mutate(spanish_mackerel_release_fl = if_else(ST == 12,
                                              `SPANISH MACKEREL_release`,as.numeric(0)),
         spanish_mackerel_released_fl = if_else(ST == 12,
                                             `SPANISH MACKEREL_released`,as.numeric(0))) %>% 
  mutate(delta_sm_fl = spanish_mackerel_release_fl - spanish_mackerel_released_fl) %>% 
...
#ty2_17_cut_13_sm_se
ty2_17_cut_13_sm_fl_total <- predict(ty2.r17_sm_cut_13_fl,total = nrow(cls_17_fl))[[1]] + sum(cls_17_fl$`SPANISH MACKEREL_released`,na.rm=T)
...
mrip_all_17_2 <- read_csv("data/mrip_dm_17_all_sites.csv",
                          col_types = cols(psu_id = col_character()))
mrip_all_17_2$`SPANISH MACKEREL_released`[is.na(mrip_all_17_2$`SPANISH MACKEREL_released`)] <- 0
...
mrip_all_17_2_al <- mrip_all_17_2 %>% 
  mutate(spanish_mackerel_release_al = if_else(ST == 1,
                                             `SPANISH MACKEREL_release`,as.numeric(0)),
         spanish_mackerel_released_al = if_else(ST == 1,
                                            `SPANISH MACKEREL_released`,as.numeric(0))) %>% 
  mutate(delta_sm_al = spanish_mackerel_release_al - spanish_mackerel_released_al) %>% 
...
#ty2_17_cut_13_sm_se
ty2_17_cut_13_sm_al_total <- predict(ty2.r17_sm_cut_13_al,total = nrow(cls_17_al))[[1]] + sum(cls_17_al$`SPANISH MACKEREL_released`,na.rm=T)
