...
ty_hat_rs <- svytotal(~red_snapper_claim_fl,
                      design = tyrs)
#ty*_hat
ty_star_hat_rs <- svytotal(~red_snapper_kept_fl,
                           design = tyrs)

#t_y*
ty_star_rs <- sum(cls_17_fl$`RED SNAPPER_kept`,na.rm=T)
...
mrip_all_17_2_al <- mrip_all_17_2 %>% 
  ...
  mutate(reported_al = if_else(ST == 1,
                               reported,as.integer(0)))
# mrip_all_17_2_al <- mrip_all_17_2_al %>% 
#   left_join(
...
ty2_17_cut_13_rs_al_total <- predict(ty2.r17_rs_cut_13_al,total = nrow(cls_17_al))[[1]] + sum(cls_17_al$`RED SNAPPER_kept`,na.rm=T)
...
ty_hat_rs <- svytotal(~red_snapper_claim_al,
                      design = tyrs)
#ty*_hat
ty_star_hat_rs <- svytotal(~red_snapper_kept_al,
                           design = tyrs)

#t_y*
ty_star_rs <- sum(cls_17_al$`RED SNAPPER_kept`,na.rm=T)
