...
mrip_all_17$`SAILFISH_released`[is.na(mrip_all_17$`SAILFISH_released`)] <- 0
mrip_all_17 <- mrip_all_17 %>% 
  mutate(`SAILFISH_released` = as.numeric(`SAILFISH_released`)) %>% 
  mutate(sailfish_release_fl = if_else(ST == 12,
                                     `SAILFISH_release`,as.numeric(0)),
         sailfish_released_fl = if_else(ST == 12,
                                    `SAILFISH_released`,as.numeric(0))) %>% 
  mutate(delta_rs = sailfish_release_fl - sailfish_released_fl) %>% 
...
ty2_17_rs_total <- predict(ty2.r17_rs,total = nrow(cls_17))[[1]] + 
  sum(as.numeric(cls_17$`SAILFISH_released`),na.rm=T)
...
mrip_all_17_1 <- read_csv("data/mrip_dm_17_all_sites.csv",
                          col_types = cols(psu_id = col_character()))
mrip_all_17_1$`SAILFISH_released`[is.na(mrip_all_17_1$`SAILFISH_released`)] <- 0
mrip_all_17_1$`SAILFISH_released` <- as.numeric(mrip_all_17_1$`SAILFISH_released`)
mrip_all_17_1 <- mrip_all_17_1 %>% 
  mutate(delta_rs = `SAILFISH_release` - `SAILFISH_released`)
...
ty2_17_cut_13_rs_fl_total <- predict(ty2.r17_rs_cut_13_fl,total = nrow(cls_17_fl))[[1]] + 
  sum(as.numeric(cls_17_fl$`SAILFISH_released`),na.rm=T)
...
mrip_all_17_2 <- read_csv("data/mrip_dm_17_all_sites.csv",
                          col_types = cols(psu_id = col_character()))
mrip_all_17_2$`SAILFISH_released`[is.na(mrip_all_17_2$`SAILFISH_released`)] <- 0
mrip_all_17_2$`SAILFISH_released` <- as.numeric(mrip_all_17_2$`SAILFISH_released`)
...
ty2_17_cut_13_rs_al_total <- predict(ty2.r17_rs_cut_13_al,total = nrow(cls_17_al))[[1]] + 
  sum(as.numeric(cls_17_al$`SAILFISH_released`),na.rm=T)
