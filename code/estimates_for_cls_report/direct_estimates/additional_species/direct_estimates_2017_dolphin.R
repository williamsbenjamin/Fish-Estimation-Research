...
mrip_all_17$`DOLPHIN_kept`[is.na(mrip_all_17$`DOLPHIN_kept`)] <- 0
mrip_all_17 <- mrip_all_17 %>% 
  mutate(`DOLPHIN_kept` = as.numeric(`DOLPHIN_kept`)) %>% 
  mutate(delta_rs = `DOLPHIN_claim` - `DOLPHIN_kept`) 
...
ty2_17_rs_total <- predict(ty2.r17_rs,total = nrow(cls_17))[[1]] + sum(cls_17$`DOLPHIN_kept`,na.rm=T)
...
mrip_all_17_1 <- read_csv("data/mrip_dm_17_all_sites.csv",
                          col_types = cols(psu_id = col_character()))
mrip_all_17_1$`DOLPHIN_kept`[is.na(mrip_all_17_1$`DOLPHIN_kept`)] <- 0
mrip_all_17_1$`DOLPHIN_kept` <- as.numeric(mrip_all_17_1$`DOLPHIN_kept`)
mrip_all_17_1 <- mrip_all_17_1 %>% 
  mutate(delta_rs = `DOLPHIN_claim` - `DOLPHIN_kept`)
mrip_all_17_1_fl <- mrip_all_17_1 %>% 
  mutate(red_snapper_claim_fl = if_else(ST == 12,
                                        `DOLPHIN_claim`,as.integer(0)),
         red_snapper_kept_fl = if_else(ST == 12,
                                       `DOLPHIN_kept`,as.numeric(0))) %>% 
...
ty2_17_cut_13_rs_fl_total <- predict(ty2.r17_rs_cut_13_fl,total = nrow(cls_17_fl))[[1]] + sum(cls_17_fl$`DOLPHIN_kept`,na.rm=T)
...
mrip_all_17_2 <- read_csv("data/mrip_dm_17_all_sites.csv",
                          col_types = cols(psu_id = col_character()))
mrip_all_17_2$`DOLPHIN_kept`[is.na(mrip_all_17_2$`DOLPHIN_kept`)] <- 0
mrip_all_17_2$`DOLPHIN_kept` <- as.numeric(mrip_all_17_2$`DOLPHIN_kept`)
...
ty2_17_cut_13_rs_al_total <- predict(ty2.r17_rs_cut_13_al,total = nrow(cls_17_al))[[1]] + sum(cls_17_al$`DOLPHIN_kept`,na.rm=T)
