...
mrip_all_17$`LITTLEHEAD PORGY_kept`[is.na(mrip_all_17$`LITTLEHEAD PORGY_kept`)] <- 0
mrip_all_17 <- mrip_all_17 %>% 
  mutate(`LITTLEHEAD PORGY_kept` = as.numeric(`LITTLEHEAD PORGY_kept`)) %>% 
  mutate(littlehead_porgy_claim_fl = if_else(ST == 12,
                                     `LITTLEHEAD PORGY_claim`,as.integer(0)),
         littlehead_porgy_kept_fl = if_else(ST == 12,
                                    `LITTLEHEAD PORGY_kept`,0)) %>% 
  mutate(delta_rs = littlehead_porgy_claim_fl - littlehead_porgy_kept_fl) %>% 
...
ty2_17_rs_total <- predict(ty2.r17_rs,total = nrow(cls_17))[[1]] + sum(cls_17$`LITTLEHEAD PORGY_kept`,na.rm=T)
...
mrip_all_17_1 <- read_csv("data/mrip_dm_17_all_sites.csv",
                          col_types = cols(psu_id = col_character()))
mrip_all_17_1$`LITTLEHEAD PORGY_kept`[is.na(mrip_all_17_1$`LITTLEHEAD PORGY_kept`)] <- 0
mrip_all_17_1$`LITTLEHEAD PORGY_kept` <- as.numeric(mrip_all_17_1$`LITTLEHEAD PORGY_kept`)
mrip_all_17_1 <- mrip_all_17_1 %>% 
  mutate(delta_rs = `LITTLEHEAD PORGY_claim` - `LITTLEHEAD PORGY_kept`)
mrip_all_17_1_fl <- mrip_all_17_1 %>% 
  mutate(red_snapper_claim_fl = if_else(ST == 12,
                                        `LITTLEHEAD PORGY_claim`,as.integer(0)),
         red_snapper_kept_fl = if_else(ST == 12,
                                       `LITTLEHEAD PORGY_kept`,as.numeric(0))) %>% 
...
ty2_17_cut_13_rs_fl_total <- predict(ty2.r17_rs_cut_13_fl,total = nrow(cls_17_fl))[[1]] + 
  sum(cls_17_fl$`LITTLEHEAD PORGY_kept`,na.rm=T)
...
mrip_all_17_2 <- read_csv("data/mrip_dm_17_all_sites.csv",
                          col_types = cols(psu_id = col_character()))
mrip_all_17_2$`LITTLEHEAD PORGY_kept`[is.na(mrip_all_17_2$`LITTLEHEAD PORGY_kept`)] <- 0
mrip_all_17_2$`LITTLEHEAD PORGY_kept` <- as.numeric(mrip_all_17_2$`LITTLEHEAD PORGY_kept`)
...
ty2_17_cut_13_rs_al_total <- predict(ty2.r17_rs_cut_13_al,total = nrow(cls_17_al))[[1]] + 
  sum(cls_17_al$`LITTLEHEAD PORGY_kept`,na.rm=T)
