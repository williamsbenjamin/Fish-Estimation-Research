...
mrip_all_17$`COBIA_kept`[is.na(mrip_all_17$`COBIA_kept`)] <- 0
mrip_all_17 <- mrip_all_17 %>% 
  mutate(`COBIA_kept` = as.numeric(`COBIA_kept`)) %>% 
  mutate(delta_rs = `COBIA_claim` - `COBIA_kept`) 
...
ty2_17_rs_total <- predict(ty2.r17_rs,total = nrow(cls_17))[[1]] + sum(cls_17$`COBIA_kept`,na.rm=T)
...
ty2_17_cut_13_rs_fl_total <- predict(ty2.r17_rs_cut_13_fl,total = nrow(cls_17_fl))[[1]] + sum(cls_17_fl$`COBIA_kept`,na.rm=T)
...
mrip_all_17_2 <- read_csv("data/mrip_dm_17_all_sites.csv",
                          col_types = cols(psu_id = col_character()))
mrip_all_17_2$`COBIA_kept`[is.na(mrip_all_17_2$`COBIA_kept`)] <- 0
...
ty2_17_cut_13_rs_al_total <- predict(ty2.r17_rs_cut_13_al,total = nrow(cls_17_al))[[1]] + sum(cls_17_al$`COBIA_kept`,na.rm=T)
