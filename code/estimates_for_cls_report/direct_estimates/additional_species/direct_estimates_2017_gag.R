...
mrip_all_17$`GAG_kept`[is.na(mrip_all_17$`GAG_kept`)] <- 0
mrip_all_17 <- mrip_all_17 %>% 
  mutate(`GAG_kept` = as.numeric(`GAG_kept`)) %>% 
  mutate(delta_rs = `GAG_claim` - `GAG_kept`) 
...
ty2_17_cut_13_rs_fl_total <- predict(ty2.r17_rs_cut_13_fl,total = nrow(cls_17_fl))[[1]] + sum(cls_17_fl$`GAG_kept`,na.rm=T)
...
mrip_all_17_2 <- read_csv("data/mrip_dm_17_all_sites.csv",
                          col_types = cols(psu_id = col_character()))
mrip_all_17_2$`GAG_kept`[is.na(mrip_all_17_2$`GAG_kept`)] <- 0
...
ty2_17_cut_13_rs_al_total <- predict(ty2.r17_rs_cut_13_al,total = nrow(cls_17_al))[[1]] + sum(cls_17_al$`GAG_kept`,na.rm=T)
