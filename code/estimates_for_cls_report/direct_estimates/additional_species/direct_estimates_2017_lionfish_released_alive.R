...
mrip_all_17$`LIONFISH_released`[is.na(mrip_all_17$`LIONFISH_released`)] <- 0
mrip_all_17 <- mrip_all_17 %>% 
  mutate(`LIONFISH_released` = as.numeric(`LIONFISH_released`)) %>% 
  mutate(lionfish_release_fl = if_else(ST == 12,
                                          `LIONFISH_release`,as.integer(0)),
         lionfish_released_fl = if_else(ST == 12,
                                         `LIONFISH_released`,0)) %>% 
  mutate(delta_rs = lionfish_release_fl - lionfish_released_fl) %>% 
...
ty2_17_rs_total <- predict(ty2.r17_rs,total = nrow(cls_17))[[1]] + sum(cls_17$`LIONFISH_released`,na.rm=T)
...
mrip_all_17_1 <- read_csv("data/mrip_dm_17_all_sites.csv",
                          col_types = cols(psu_id = col_character()))
mrip_all_17_1$`LIONFISH_released`[is.na(mrip_all_17_1$`LIONFISH_released`)] <- 0
mrip_all_17_1$`LIONFISH_released` <- as.numeric(mrip_all_17_1$`LIONFISH_released`)
mrip_all_17_1 <- mrip_all_17_1 %>% 
  mutate(delta_rs = `LIONFISH_release` - `LIONFISH_released`)
mrip_all_17_1_fl <- mrip_all_17_1 %>% 
  mutate(red_snapper_release_fl = if_else(ST == 12,
                                          `LIONFISH_release`,as.integer(0)),
         red_snapper_released_fl = if_else(ST == 12,
                                           `LIONFISH_released`,as.numeric(0))) %>% 
...
ty2_17_cut_13_rs_fl_total <- predict(ty2.r17_rs_cut_13_fl,total = nrow(cls_17_fl))[[1]] + 
  sum(cls_17_fl$`LIONFISH_released`,na.rm=T)
...
mrip_all_17_2 <- read_csv("data/mrip_dm_17_all_sites.csv",
                          col_types = cols(psu_id = col_character()))
mrip_all_17_2$`LIONFISH_released`[is.na(mrip_all_17_2$`LIONFISH_released`)] <- 0
mrip_all_17_2$`LIONFISH_released` <- as.numeric(mrip_all_17_2$`LIONFISH_released`)
...
ty2_17_cut_13_rs_al_total <- predict(ty2.r17_rs_cut_13_al,total = nrow(cls_17_al))[[1]] + 
  sum(as.numeric(cls_17_al$`LIONFISH_released`),na.rm=T)
