...
mrip_all_17$`SCAMP_kept`[is.na(mrip_all_17$`SCAMP_kept`)] <- 0
mrip_all_17 <- mrip_all_17 %>% 
  mutate(`SCAMP_kept` = as.numeric(`SCAMP_kept`)) %>% 
  mutate(delta_rs = `SCAMP_claim` - `SCAMP_kept`) 
cls_17$`SCAMP_kept` <- as.numeric(cls_17$`SCAMP_kept`)
...
ty2_17_rs_total <- predict(ty2.r17_rs,total = nrow(cls_17))[[1]] + sum(cls_17$`SCAMP_kept`,na.rm=T)
...
mrip_all_17_1$`SCAMP_kept`[is.na(mrip_all_17_1$`SCAMP_kept`)] <- 0
mrip_all_17_1 <- mrip_all_17_1 %>% 
  mutate(delta_rg = `SCAMP_claim` - `SCAMP_kept`) %>%
  mutate(non_reporter_mrip_claim_rg = if_else(reported == 0,
                                              `SCAMP_claim`,as.integer(0)))
...
#ty2_17_cut_13_rg_se
ty2_17_cut_13_rg_fl_total <- predict(ty2.r17_rg_cut_13_fl,total = nrow(cls_17_fl))[[1]] + sum(cls_17_fl$`SCAMP_kept`,na.rm=T)
...
mrip_all_17_1 <- read_csv("data/mrip_dm_17_all_sites.csv",
                          col_types = cols(psu_id = col_character()))
mrip_all_17_1$`SCAMP_kept`[is.na(mrip_all_17_1$`SCAMP_kept`)] <- 0
mrip_all_17_1$`SCAMP_kept` <- as.numeric(mrip_all_17_1$`SCAMP_kept`)
mrip_all_17_1 <- mrip_all_17_1 %>% 
  mutate(delta_rs = `SCAMP_claim` - `SCAMP_kept`)
mrip_all_17_1_fl <- mrip_all_17_1 %>% 
...
ty2_17_cut_13_rs_fl_total <- predict(ty2.r17_rs_cut_13_fl,total = nrow(cls_17_fl))[[1]] + 
  sum(cls_17_fl$`SCAMP_kept`,na.rm=T)
...
mrip_all_17_2 <- read_csv("data/mrip_dm_17_all_sites.csv",
                          col_types = cols(psu_id = col_character()))
mrip_all_17_2$`SCAMP_kept`[is.na(mrip_all_17_2$`SCAMP_kept`)] <- 0
mrip_all_17_2$`SCAMP_kept` <- as.numeric(mrip_all_17_2$`SCAMP_kept`)
...
ty2_17_cut_13_rs_al_total <- predict(ty2.r17_rs_cut_13_al,total = nrow(cls_17_al))[[1]] + 
  sum(cls_17_al$`SCAMP_kept`,na.rm=T)
