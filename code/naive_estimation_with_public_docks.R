...
mrip_cls_all<-mrip_cls_all %>%
  mutate(delta = total_claim_mrip - total_kept_cls,na.rm=T)

mrip_cls_all <- mrip_cls_all %>% 
  mutate(reported = if_else(!is.na(delta)
                            ,1,0),
         non_reporter_mrip_claim = if_else(is.na(delta)
                                           ,total_claim_mrip,NULL))
...
predict(tyc,total=sum(cls_16$total_kept_cls))
