...
library(ggthemes)
library(lubridate)
tidy_all_matches <- read_csv("data/2017_all_possible_matches_mrip_cls.csv",
                             col_names = T,
                             cols(
                               tripID = col_double()
                             )
...
tidy_all_matches_docks <- tidy_all_matches_docks %>%
  mutate(diff_total_release = total_released_cls - total_release_mrip)

write_csv(tidy_all_matches_docks,"data/tidy_all_matches_docks_2017.csv")
