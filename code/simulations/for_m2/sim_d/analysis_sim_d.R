library(tidyverse)
d_dat <- read_rds("code/simulations/for_m2/sim_d/sim_results_D_2-19.rds")
pop_tot_d <- 375173
...
#Rel Bias
d_dat %>% 
  summarise(yp_rb = (mean(t_yp_est) - pop_tot_d) / pop_tot_d,
            yc_rb = (mean(t_yc_est) - pop_tot_d) / pop_tot_d,
            y2_rb = (mean(t_y2_est)- pop_tot_d) / pop_tot_d,
            ydiff_rb = (mean(t_ydiff_est)- pop_tot_d) / pop_tot_d,
            ynew_rb = (mean(t_ynew_est)- pop_tot_d) / pop_tot_d)

#Bias
d_dat %>% 
  summarise(yp_b = (mean(t_yp_est) - pop_tot_d),
            yc_b = (mean(t_yc_est) - pop_tot_d),
            y2_b = (mean(t_y2_est)- pop_tot_d),
            ydiff_b = (mean(t_ydiff_est)- pop_tot_d),
            ynew_b = (mean(t_ynew_est)- pop_tot_d))
