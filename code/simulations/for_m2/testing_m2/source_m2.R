...
str(pop)
#for(jj in 1:20){
big_loop <- function(nest_pop.,
                     cls17.,
                     pop.) { # Need function arguments
###################################
...
parallelStartSocket(1) #try on my laptop with 2
parallelGetOptions()
.libPaths("/hpc/applications/r/3.4.1/lib64/R/library")
...
big_loop_results <- parallelMap(big_loop,vars = list(cls17. = cls17,
                                              nest_pop. = nest_pop,
                                              pop. = pop))
parallelStop()
...
saveRDS(sim_results,
        file = "/users/benjamin/scratch/dissertation/sim_results.rds")
