library(tidyverse)
c_dat <- read_rds("code/simulations/for_m2/sim_c/sim_results_c_2-19.rds")
pop_tot_c <- 373690
...
#Rel Bias
c_dat %>% 
  summarise(yp_rb = (mean(t_yp_est) - pop_tot_c) / pop_tot_c,
            yc_rb = (mean(t_yc_est) - pop_tot_c) / pop_tot_c,
            y2_rb = (mean(t_y2_est)- pop_tot_c) / pop_tot_c,
            ydiff_rb = (mean(t_ydiff_est)- pop_tot_c) / pop_tot_c,
            ynew_rb = (mean(t_ynew_est)- pop_tot_c) / pop_tot_c)

#Bias
c_dat %>% 
  summarise(yp_b = (mean(t_yp_est) - pop_tot_c),
            yc_b = (mean(t_yc_est) - pop_tot_c),
            y2_b = (mean(t_y2_est)- pop_tot_c),
            ydiff_b = (mean(t_ydiff_est)- pop_tot_c),
            ynew_b = (mean(t_ynew_est)- pop_tot_c))
