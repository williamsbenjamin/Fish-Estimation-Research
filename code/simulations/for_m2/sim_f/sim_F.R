...
big_loop <- function() { # Need function arguments
  ...
  #make too few matches (20% fewer matches)
  ...
  ########
  identified_matches <- nrow(filter(tidy_all_matches_one_per,
                                    rl_score > cutoff))
  ...
  rates <- matched_samp %>% 
    mutate(
      match_status = case_when( 
        ...
        match == 0 & true_match_id_mrip != 0  ~ "false-neg",
    ...
    summarize(fp.rate = sum(match_status == "false-pos") / 
              ...
              mm.rate = sum(match_status == "mismatch" ) / 
                sum(true_match_id_mrip != 0),
              tn.rate = sum(match_status == "true-neg") /
                sum(true_match_id_mrip == 0)
    ) 
  ...
  return(list(t_yp_est = typ_total,
              ...
              mm_rate = mm.rate,
              tn_rate = tn.rate,
              true_match = true_match_s,
              identified_match = identified_matches))
...
saveRDS(sim_results,
        file = "/users/benjamin/scratch/dissertation/sim_results_f_2-19.rds")
