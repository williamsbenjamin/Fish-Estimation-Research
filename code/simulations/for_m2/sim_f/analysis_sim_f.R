library(tidyverse)
f_dat <- read_rds("code/simulations/for_m2/sim_f/sim_results_f_2-19.rds")
pop_tot_f <- 376834
...
#Rel Bias
f_dat %>% 
  summarise(yp_rb = (mean(t_yp_est) - pop_tot_f)/pop_tot_f,
            yc_rb = (mean(t_yc_est) - pop_tot_f)/pop_tot_f,
            y2_rb = (mean(t_y2_est)- pop_tot_f)/pop_tot_f,
            ydiff_rb = (mean(t_ydiff_est)- pop_tot_f)/pop_tot_f,
            ynew_rb = (mean(t_ynew_est)- pop_tot_f)/pop_tot_f)
#Bias
f_dat %>% 
  summarise(yp_b = (mean(t_yp_est) - pop_tot_f),
            yc_b = (mean(t_yc_est) - pop_tot_f),
            y2_b = (mean(t_y2_est)- pop_tot_f),
            ydiff_b = (mean(t_ydiff_est)- pop_tot_f),
            ynew_b = (mean(t_ynew_est)- pop_tot_f))
