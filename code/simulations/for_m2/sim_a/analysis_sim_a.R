library(tidyverse)
a_dat <- read_rds("code/simulations/for_m2/sim_a/sim_results_A_2-19.rds")
pop_tot <- 372895
...
#Rel Bias
a_dat %>% 
  summarise(yp_rb = (mean(t_yp_est) - pop_tot) / pop_tot,
            yc_rb = (mean(t_yc_est) - pop_tot) / pop_tot,
            y2_rb = (mean(t_y2_est)- pop_tot) / pop_tot,
            ydiff_rb = (mean(t_ydiff_est)- pop_tot) / pop_tot,
            ynew_rb = (mean(t_ynew_est)- pop_tot) / pop_tot)

#Bias
a_dat %>% 
  summarise(yp_b = (mean(t_yp_est) - pop_tot),
            yc_b = (mean(t_yc_est) - pop_tot),
            y2_b = (mean(t_y2_est)- pop_tot),
            ydiff_b = (mean(t_ydiff_est)- pop_tot),
            ynew_b = (mean(t_ynew_est)- pop_tot))
